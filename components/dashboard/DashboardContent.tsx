...
export default function DashboardContent({
  ...
  const summaryData = useMemo(() => {
    const currentMonth = currentMonthEmployees.length
    const totalCost = currentMonthEmployees.reduce((sum, emp) => 
    ...
    return {
      ...
      plannedHires: 25, // TODO: 予定データから取得
      totalCost: totalCost,
      avgCost: avgCost
    }
  }, [currentMonthEmployees])
  ...
  const monthlyTrendData = useMemo(() => {
    ...
    return Object.entries(monthlyData)
      ...
      .map(([month, data]) => ({
        ...
        予定: data.planned || Math.floor(Math.random() * 10) + 20, // TODO: 実際の予定データ
        実績: data.actual
      }))
  }, [historicalData])
